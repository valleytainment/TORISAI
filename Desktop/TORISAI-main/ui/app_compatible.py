"""
TORIS AI - UI-Backend Integration (Gradio Compatible)
Implements full integration between the UI and backend API with compatible Gradio components
"""
import os
import gradio as gr
import httpx
        ...
        with gr.Blocks(title="TORISAI", css=CUSTOM_CSS, theme=gr.themes.Soft()) as self.ui:
            with gr.Row():
                ...
                with gr.Column(scale=2, min_width=300):
                    ...
                    # Agent status box (using a div with custom CSS instead of Box)
                    with gr.Group(elem_classes="agent-status-box"):
                        with gr.Row():
                            # Status indicator (using HTML instead of Label)
                            gr.HTML("<div id='status_indicator'>ACTIVE</div>", elem_id="status_container")
                        ...
                        with gr.Row():
                            with gr.Column():
                                self.mode_label = gr.Markdown(f"**Mode:** {self.current_agent}")
                            with gr.Column():
                                self.steps_label = gr.Markdown(f"**Steps:** {self.steps_count}")
                    
                    with gr.Tabs() as self.right_tabs:
                        with gr.TabItem("Memory View"):
                            self.memory_view = gr.Textbox(
                                value=self.memory_content,
                                label="",
                                interactive=False,
                                elem_classes="memory-view"
                            )
                        ...
                        with gr.TabItem("Code Execution"):
                            ...
                            self.code_output_view = gr.Textbox(
                    ...
                    with gr.Row():
                        ...
                        self.run_command_btn = gr.Button(">", variant="primary", scale=1)
